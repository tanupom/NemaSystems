...
def StartCamerasFunction(num,s,s2,pi,a,r,f):
    ...
    if(pi.value == 0): #Timelapse
        ...
        print(Filename4)
        try:
            os.mkdir("/media/infinitysystem2/InternamDrive/Videos/TimelapseFolders/"+Filename1+"/")
        except:
            pass
        try:
            os.mkdir("/media/infinitysystem2/InternamDrive/Videos/TimelapseFolders/"+Filename2+"/")
        except:
            pass
        try:
            os.mkdir("/media/infinitysystem2/InternamDrive/Videos/TimelapseFolders/"+Filename3+"/")
        except:
            pass
        try:
            os.mkdir("/media/infinitysystem2/InternamDrive/Videos/TimelapseFolders/"+Filename4+"/")
        except:
            pass
...
! jpegdec \
! video/x-raw \
! videocrop top=325 left=0 right=0 bottom=325 \
! videoconvert \
! videorate \
...
! videoconvert \
! jpegenc \
! multifilesink location=/media/infinitysystem2/InternamDrive/Videos/TimelapseFolders/"+Filename1+"/"+"Frame_%06d.jpg \
...
! videoconvert \
! jpegenc \
! multifilesink location=/media/infinitysystem2/InternamDrive/Videos/TimelapseFolders/"+Filename2+"/"+"Frame_%06d.jpg \
...
! videoconvert \
! jpegenc \
! multifilesink location=/media/infinitysystem2/InternamDrive/Videos/TimelapseFolders/"+Filename3+"/"+"Frame_%06d.jpg \
...
! videoconvert \
! jpegenc \
! multifilesink location=/media/infinitysystem2/InternamDrive/Videos/TimelapseFolders/"+Filename4+"/"+"Frame_%06d.jpg"

    elif(pi.value == 1):
        DEFAULT_PIPELINE = "v4l2src device=/dev/video6 \
! image/jpeg,width=(int)3264, height=(int)2448 \
! jpegdec \
! video/x-raw \
! videocrop top=325 left=0 right=0 bottom=325 \
...
! h265parse \
! qtmux \
! filesink location=/media/infinitysystem2/InternamDrive/Videos/Recorded_Video/"+Filename1+" \
v4l2src device=/dev/video4 \
! image/jpeg,width=(int)3264, height=(int)2448 \
! jpegdec \
! video/x-raw \
...
! h265parse \
! qtmux \
! filesink location=/media/infinitysystem2/InternamDrive/Videos/Recorded_Video/"+Filename2+" \
v4l2src device=/dev/video2 \
! image/jpeg,width=(int)3264, height=(int)2448 \
! jpegdec \
! video/x-raw \
...
! h265parse \
! qtmux \
! filesink location=/media/infinitysystem2/InternamDrive/Videos/Recorded_Video/"+Filename3+" \
...
! h265parse \
! qtmux \
! filesink location=/media/infinitysystem2/InternamDrive/Videos/Recorded_Video/"+Filename4
    else:
        DEFAULT_PIPELINE = "v4l2src device=/dev/video6 \
    ! image/jpeg,width=(int)3264, height=(int)2448 \
    ! jpegdec \
   ...
   # args = vars(ap.parse_args())
    ...
    time.sleep(.2)
    call(["v4l2-ctl", "-d", "/dev/video0", "-c", "focus_absolute=400"])# min=1 max=1023 step=1 default=1 value=528
    call(["v4l2-ctl", "-d", "/dev/video2", "-c", "focus_absolute=432"])
    call(["v4l2-ctl", "-d", "/dev/video4", "-c", "focus_absolute=432"])
    call(["v4l2-ctl", "-d", "/dev/video6", "-c", "focus_absolute=450"])
    time.sleep(.2)
    
    while(abs(time.time()-t)<s.value+s2.value + 10 and num.value == 0.0):
        ...
        if(time.time()-t<s.value):
            if(time.time()-t<5 and s.value != 0 and s.value != 90001):
                ...
                if(CameraData['Camera2']=="true"):
                    kit.motor2.throttle = 1
                if(CameraData['Camera3']=="true"):
                    kit.motor3.throttle = 1
                if(CameraData['Camera4']=="true"):
                    kit.motor4.throttle = 1
                call(["v4l2-ctl", "-d", "/dev/video0", "-c", "focus_absolute=400"])# min=1 max=1023 step=1 default=1 value=528
                call(["v4l2-ctl", "-d", "/dev/video2", "-c", "focus_absolute=432"])
                call(["v4l2-ctl", "-d", "/dev/video4", "-c", "focus_absolute=528"])
                call(["v4l2-ctl", "-d", "/dev/video6", "-c", "focus_absolute=450"])
                time.sleep(.2)
                call(["v4l2-ctl", "-d", "/dev/video0", "-c", "brightness=10"]) # min=-64 max=64 step=1 default=0 value=0
                call(["v4l2-ctl", "-d", "/dev/video2", "-c", "brightness=10"])
                call(["v4l2-ctl", "-d", "/dev/video4", "-c", "brightness=10"])
                call(["v4l2-ctl", "-d", "/dev/video6", "-c", "brightness=10"])
                time.sleep(.2)
                ...
                time.sleep(.2)
                call(["v4l2-ctl", "-d", "/dev/video0", "-c", "sharpness=3"])# min=0 max=6 step=1 default=3 value=3
                call(["v4l2-ctl", "-d", "/dev/video2", "-c", "sharpness=3"])
                call(["v4l2-ctl", "-d", "/dev/video4", "-c", "sharpness=3"])
                call(["v4l2-ctl", "-d", "/dev/video6", "-c", "sharpness=3"])
                time.sleep(.2)
                call(["v4l2-ctl", "-d", "/dev/video0", "-c", "backlight_compensation=1"])# min=0 max=2 step=1 default=1 value=1
                call(["v4l2-ctl", "-d", "/dev/video2", "-c", "backlight_compensation=1"])
                call(["v4l2-ctl", "-d", "/dev/video4", "-c", "backlight_compensation=1"])
                call(["v4l2-ctl", "-d", "/dev/video6", "-c", "backlight_compensation=1"])
                time.sleep(.2)
                ...
                call(["v4l2-ctl", "-d", "/dev/video0", "-c", "backlight_compensation=1"])# min=0 max=2 step=1 default=1 value=1
                call(["v4l2-ctl", "-d", "/dev/video2", "-c", "backlight_compensation=1"])
                call(["v4l2-ctl", "-d", "/dev/video4", "-c", "backlight_compensation=2"])
                call(["v4l2-ctl", "-d", "/dev/video6", "-c", "backlight_compensation=1"])
                time.sleep(.2)
                call(["v4l2-ctl", "-d", "/dev/video0", "-c", "gamma=100"])# min=72 max=500 step=1 default=100 value=0
                call(["v4l2-ctl", "-d", "/dev/video2", "-c", "gamma=100"])
